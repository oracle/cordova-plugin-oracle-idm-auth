...
var IdmAuthFlows = function() {
  ...
  var AuthenticationFlow = function(authFlowKey)
  {
     ...
     *                            <li>onRejected - will receive the error object describing the error.</ul>
     */
     ...
     *                   <ul><li>e.g. For HTTPBasicAuthentication, if offlineAuthAllowed is true - {Authorization: '<base64Encoded credentials>', customHeader1: 'headerValue1', ... }
     *                   <li>e.g. For OAuthAuthentication, {oauth_access_token1:{name:<name>, OAuthTokenScopes:[...], expires:<expiryDate>, value:<auth-token>}}</ul>
     *                   Headers are returned only if they exists. If no headers are available an empty object is returned.
     *                   <li>onRejected - will receive the error object describing the error.</ul>
     */
    this.getHeaders = function(fedAuthSecuredUrl) {
      return new Promise(function (resolve, reject) {
        exec(resolve, reject, TAG, 'getHeaders', [authFlowKey, fedAuthSecuredUrl]);
      });
    };
    /**
  ...
  return {
     ...
     * </ul>
     * @return {Object} promise - onFulfilled - receives an AuthenticationFlow object which can be used for login, logout etc.
     */
